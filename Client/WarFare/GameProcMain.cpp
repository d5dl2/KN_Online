...
void CGameProcMain::InitPlayerPosition(const __Vector3& vPos) // ÇÃ·¹ÀÌ¾î À§Ä¡ ÃÊ±âÈ­.. ÀÏÀ¸ÄÑ ¼¼¿ì°í, ±âº»µ¿ÀÛÀ» ÃëÇÏ°Ô ÇÑ´Ù.
{
...
bool CGameProcMain::ProcessPacket(Packet& pkt)
{
	...
	case WIZ_WEATHER:
		...
		return true;
	case WIZ_USER_INOUT:												// ´Ù¸¥ À¯Àú ÀÎ/¾Æ¿ô..
		this->MsgRecv_UserInOut(pkt);
		return true;
	case WIZ_REGIONCHANGE:										// Ã·¿¡ ·Î±×¿ÂÇÏ¸é ±× ÁÖº¯ Áö¿ªÀÇ Ä³¸¯ÅÍµé ¾÷µ¥ÀÌÆ®...
		this->MsgRecv_UserInAndRequest(pkt);
		return true;
	case WIZ_REQ_USERIN:										// ¼­¹ö¿¡ ¿äÃ»ÇÑ UserIn ¿¡ ´ëÇÑ ÀÚ¼¼ÇÑ Á¤º¸ ¹Þ±â..
		this->MsgRecv_UserInRequested(pkt);						// 
		return true;
	case WIZ_NPC_REGION:										// Ã·¿¡ ·Î±×¿ÂÇÏ¸é ±× ÁÖº¯ Áö¿ªÀÇ Ä³¸¯ÅÍµé ¾÷µ¥ÀÌÆ®...
		this->MsgRecv_NPCInAndRequest(pkt);
		return true;
	case WIZ_REQ_NPCIN:											// ¼­¹ö¿¡ ¿äÃ»ÇÑ UserIn ¿¡ ´ëÇÑ ÀÚ¼¼ÇÑ Á¤º¸ ¹Þ±â..
		this->MsgRecv_NPCInRequested(pkt);						// 
		return true;
	case WIZ_NPC_INOUT:												// NPC ÀÎ/¾Æ¿ô..
		this->MsgRecv_NPCInOut(pkt);
		return true;
	case WIZ_ATTACK:
		this->MsgRecv_Attack(pkt);
		return true;
	case WIZ_NPC_MOVE:												// NPC ¿òÁ÷ÀÓ ÆÐÅ¶..
	...
	case WIZ_TARGET_HP:
		...
		return true;
	case WIZ_ITEM_MOVE:
		this->MsgRecv_ItemMove(pkt);				// Item Move¿¡ ´ëÇÑ ÀÀ´ä..
		return true;
	case WIZ_ITEM_DROP:
	...
	case WIZ_ITEM_TRADE:
		...
		return true;
	case WIZ_ITEM_GET:
		this->MsgRecv_ItemDroppedGetResult(pkt);					// ¶¥¿¡ ¶³¾îÁø ¾ÆÀÌÅÛ ¸Ô±â °á°ú..
		return true;
	case WIZ_REPAIR_NPC:
	...
	case WIZ_CHAT_TARGET:
	{
		...
		else
		{
			...
			m_pWhisperMgr->IncomingMessage(szID, "");
			eCM = N3_CHAT_PRIVATE;
		}
	}
	return true;
	case WIZ_CONCURRENTUSER: // µ¿½Ã Á¢¼ÓÀÚ¼ö ...
	{
		int iUserCount = pkt.read<int16_t>();		// ID ¹®ÀÚ¿­ ±æÀÌ..
	...
	case WIZ_KNIGHTS_LIST:
		...
		return true;
	case WIZ_CONTINOUS_PACKET: // ¾ÐÃàµÈ µ¥ÀÌÅÍ ÀÌ´Ù... ÇÑ¹ø ´õ ÆÄ½ÌÇØ¾ß ÇÑ´Ù!!!
		this->MsgRecv_ContinousPacket(pkt);
		return true;
	case WIZ_WAREHOUSE:	// º¸°üÇÔ..
		this->MsgRecv_WareHouse(pkt);			// º¸°üÇÔ °ü·Ã ÆÐÅ¶..
		return true;
	...
	case WIZ_WARP_LIST:
		...
		//			return true;
	case WIZ_CORPSE: //regenÀ» ÇÏ¿© ÁÖÀ§ À¯Àú¿¡°Ô ½ÃÃ¼ÀÓÀ» ¾Ë¸°´Ù.
	...
	case WIZ_EFFECT:
	{
		...
		{
			char szBuffer[128];
			sprintf(szBuffer, "WIZ_EFFECT packet, skillID not found: %d", skillId);
...
void CGameProcMain::MsgSend_Continous()						// Æ¯Á¤ Á¶°Ç(?)ÇÏ¿¡¼­ ¼­¹ö¿¡°Ô Á¤±âÀûÀ¸·Î ¸Þ½ÃÁö¸¦ º¸³¿..
{
	...
	if (fTime >= m_fMsgSendTimeMove + PACKET_INTERVAL_MOVE)					// 1ÃÊ°¡ Áö³µÀ¸¸é..	
	{
		...
		if (m_vPlayerPosSended != vPos) this->MsgSend_Move(true, true); // Á¶±ÝÀÌ¶óµµ ¿òÁ÷¿´À¸¸é ¿òÁ÷ÀÓ ÆÐÅ¶ º¸³¿..
	}

	if (false == s_pPlayer->m_bMoveContinous && fTime >= m_fMsgSendTimeRot + PACKET_INTERVAL_ROTATE) // ÇÃ·¹ÀÌ¾î°¡ Á¤ÁöÇØ ÀÖ°í.. 2ÃÊ°¡ Áö³µÀ¸¸é..
	{
		float fYaw = s_pPlayer->Yaw();
		if (fYaw != m_fPlayerYawSended) MsgSend_Rotation(); // Á¶±ÝÀÌ¶óµµ È¸ÀüÇßÀ¸¸é È¸Àü ÆÐÅ¶ º¸³¿
	}
}
...
void CGameProcMain::MsgSend_Attack(int iTargetID, float fInterval, float fDistance) // °ø°Ý ÆÐÅ¶ ³¯¸®±â - Å×ÀÌºíÀÇ °ø°Ý ÁÖ±â¸¦ °°ÀÌ Áà¼­ ÇØÅ·À» ¸·´Â´Ù.
{
	if (s_pPlayer->m_fTimeAfterDeath > 0 || s_pPlayer->IsDead())	return; // Á×Àº ³ÑÀÌ´Ù..

	uint8_t byBuff[32];												// ¹öÆÛ.. 
	int iOffset = 0;													// ¿É¼Â..

	uint8_t bySuccess = true;

	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_ATTACK);						// °ø°Ý Ä¿¸àµå..
...
void CGameProcMain::MsgSend_Move(bool bMove, bool bContinous)
{
	...
	uint8_t byBuff[64];											// ¹öÆÛ ¼³Á¤..
	int iOffset = 0;											// ¿É¼Â..

	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_MOVE);			// Ä¿¸àµå..
	CAPISocket::MP_AddWord(byBuff, iOffset, (uint16_t)(vPos.x * 10));			// ´ÙÀ½ À§Ä¡
	CAPISocket::MP_AddWord(byBuff, iOffset, (uint16_t)(vPos.z * 10));
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)(vPos.y * 10));
	CAPISocket::MP_AddWord(byBuff, iOffset, (uint16_t)(fSpeed * 10));			// ¼Óµµ 
	...
	if (true == bMove) m_fMsgSendTimeMove = CN3Base::TimeGet(); // ½Ã°£À» ±â·ÏÇÑ´Ù..
}
...
{
	if (s_pPlayer->m_iSendRegeneration >= 2) return; // ÇÑ¹ø º¸³»¸é ´Ù½Ã Á×À»¶§±îÁö ¾Èº¸³»´Â ÇÃ·¡±×
...
bool CGameProcMain::MsgSend_PartyOrForceCreate(int iPartyOrForce, const std::string& szID)
{
	...
	if (true == bIAmMember && false == bIAmLeader) return false; // ³»°¡ ÆÄÆ¼¿¡ µé¾î ÀÖ°í ¸®´õ°¡ ¾Æ´Ï¸é ½ÇÆÐ..
...
{
	if (m_pUIPartyOrForce->MemberCount() <= 0) return; // ÆÄÆ¼¿øÀÌ ¾ø´Ù..
...
void CGameProcMain::MsgSend_Weather(int iWeather, int iPercent)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_WEATHER); // -> byte - ±âÈÄ.... 0x01 - ¸¼À½.. 0x02 -  ºñ 0x03
	CAPISocket::MP_AddByte(byBuff, iOffset, iWeather); // -> byte - ±âÈÄ.... 0x01 - ¸¼À½.. 0x02 -  ºñ 0x03
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)iPercent); // int16_t -> ¸¼Àº³¯ ¾È°³, ºñ, ´« ÀÇ ¾ç ÆÛ¼¾Æ®·Î 
...
void CGameProcMain::MsgSend_Administrator(e_SubPacket_Administrator eSP, const std::string& szID)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_OPERATOR); // °ü¸®ÀÚ Àü¿ëÆÐÅ¶..
...
void CGameProcMain::MsgSend_KnightsJoinReq(bool bJoin)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_KNIGHTS_PROCESS); // °ü¸®ÀÚ Àü¿ëÆÐÅ¶..
...
bool CGameProcMain::MsgRecv_MyInfo_All(Packet& pkt)
{
	...
	s_pPlayer->m_InfoExt.iFace = pkt.read<uint8_t>(); // ¾ó±¼ ¸ð¾ç..
	s_pPlayer->m_InfoExt.iHair = pkt.read<uint8_t>(); // ¸Ó¸®Ä«¶ô
	...
	s_pPlayer->m_InfoExt.iBonusPointRemain = pkt.read<uint8_t>(); // ³²Àº º¸³Ê½º Æ÷ÀÎÆ®..
	...
	int iKnightsID = pkt.read<int16_t>(); // ¼Ò¼Ó ±â»ç´Ü ID
	e_KnightsDuty eKnightsDuty = (e_KnightsDuty)pkt.read<uint8_t>(); // ±â»ç´Ü¿¡¼­ÀÇ ±ÇÇÑ..

	// NOTE(srmeier): adding alliance ID and knight's byFlag
	int iAllianceID = pkt.read<int16_t>();
	uint8_t byFlag = pkt.read<uint8_t>();

	int iKnightNameLen = pkt.read<uint8_t>(); // ¼Ò¼Ó ±â»ç´Ü ÀÌ¸§ ±æÀÌ.
	pkt.readString(szKnightsName, iKnightNameLen);
	int iKnightsGrade = pkt.read<uint8_t>(); // ¼Ò¼Ó ±â»ç´Ü µî±Þ
	int	iKnightsRank = pkt.read<uint8_t>(); // ¼Ò¼Ó ±â»ç´Ü ¼øÀ§
	...
	s_pPlayer->m_InfoBase.iAuthority = pkt.read<uint8_t>(); //±ÇÇÑ.. 
	...
	// »óÅÂÃ¢ ¼öÄ¡¸¦ ¸ðµÎ Àû¿ë
	if (m_pUIVar->m_pPageState) m_pUIVar->m_pPageState->UpdateID(szID); // ÀÌ¸§ Àû¿ë.
	...
	for (int i = 0; i < ITEM_SLOT_COUNT; i++)				// ½½·Ô °¹¼ö¸¶Å­..
	{
		...
		e_ItemType eType = CGameProcedure::MakeResrcFileNameForUPC(pItem, &szResrcFN, &szIconFN, ePart, ePlug, s_pPlayer->m_InfoBase.eRace); // ¾ÆÀÌÅÛ¿¡ µû¸¥ ÆÄÀÏ ÀÌ¸§À» ¸¸µé¾î¼­
	...
	for (int i = 0; i < MAX_ITEM_INVENTORY; i++)				// ÀÎº¥Åä¸® °¹¼ö¸¸Å­..	
	{
		...
		e_PartPosition ePart;
		e_PlugPosition ePlug;
		e_ItemType eType = CGameProcedure::MakeResrcFileNameForUPC(pItem, NULL, &szIconFN, ePart, ePlug, s_pPlayer->m_InfoBase.eRace); // ¾ÆÀÌÅÛ¿¡ µû¸¥ ÆÄÀÏ ÀÌ¸§À» ¸¸µé¾î¼­
		if (ITEM_TYPE_UNKNOWN == eType) CLogWriter::Write("MyInfo - slot - Unknown Item");
...
bool CGameProcMain::MsgRecv_Chat(Packet& pkt)
{
	...
	e_ChatMode eCM = (e_ChatMode)pkt.read<uint8_t>();	// Ã¤ÆÃ Å¸ÀÔ
	e_Nation eNation = (e_Nation)pkt.read<uint8_t>();	// º¸³½»ç¶÷ ±¹°¡
	int iID = pkt.read<int16_t>();			// º¸³½»ç¶÷
...
{
	int iID = pkt.read<int16_t>();
	float fX = (pkt.read<uint16_t>()) / 10.0f; // Ãâ¹ßÇÏ°Å³ª ÀÌµ¿ÁßÀÏ¶§¿¡´Â ´ÙÀ½ À§Ä¡. Á¤ÁöÇÒ¶§´Â ÇöÀç À§Ä¡¸¦ ¹Þ´Â´Ù.
	float fZ = (pkt.read<uint16_t>()) / 10.0f;
	float fY = (pkt.read<int16_t>()) / 10.0f;
	float fSpeed = (pkt.read<int16_t>()) / 10.0f; // Ãâ¹ßÇÏ°Å³ª ÀÌµ¿ÁßÀÏ¶§¿¡´Â ¿òÁ÷ÀÌ´Â ¼Óµµ. Á¤ÁöÇÒ¶§´Â 0 ÀÌ ¿Â´Ù.
	uint8_t byMoveFlag = pkt.read<uint8_t>(); // ¿òÁ÷ÀÌ´Â ÇÃ·¡±×.. 0 Á¤Áö 1 Ãâ¹ß, 2 °è¼Ó ¿òÁ÷ÀÓ
...
{
	int iWeather = pkt.read<uint8_t>(); // -> byte - ±âÈÄ.... 0x01 - ¸¼À½.. 0x02 -  ºñ 0x03
	int iPercent = pkt.read<int16_t>(); // int16_t -> ¸¼Àº³¯ ¾È°³, ºñ, ´« ÀÇ ¾ç ÆÛ¼¾Æ®·Î 


	////////////////////////////////////////////////////
	// ½ºÇÇµå ÇÙ Ã¼Å©¿ë ½Ã°£....
/*	float fTimeFromServer = pkt.read<float>(); // float -> Á¤±âÀûÀ¸·Î ÁÖ´Â ½Ã°£...
...
*/
	...
	float fDelta = 1.0f;
	if (0x1 == iWeather) fDelta = 0.5f + (1.0f - fPercent) * 0.5f; // ¸¼Àº ³¯¾¾. ÆÛ¼¾Æ®´Â ¾È°³...
	else if (0x02 == iWeather) fDelta = 0.25f + (1.0f - fPercent) * 0.75f; // ºñ¿È .
	else if (0x03 == iWeather) fDelta = 0.25f + (1.0f - fPercent) * 0.75f; // ´«¿È..
...
bool CGameProcMain::MsgRecv_UserIn(Packet& pkt, bool bWithFX)
{
	...
	e_Nation eNation = (e_Nation)pkt.read<uint8_t>(); // ¼Ò¼Ó ±¹°¡. 0 ÀÌ¸é ¾ø´Ù. 1

	// ±â»ç´Ü °ü·Ã
	int iKnightsID = pkt.read<int16_t>(); // ±â»ç´Ü ID
	e_KnightsDuty eKnightsDuty = (e_KnightsDuty)pkt.read<uint8_t>(); // ¼Ò¼Ó ±¹°¡. 0 ÀÌ¸é ¾ø´Ù. 1
	...
	std::string szKnightsName;
	pkt.readString(szKnightsName, iKnightNameLen);
	int iKnightsGrade = pkt.read<uint8_t>();			// µî±Þ
	int iKnightsRank = pkt.read<uint8_t>();			// ¼øÀ§

	// NOTE(srmeier): adding mark version and cape ID
	int16_t sMarkVersion = pkt.read<int16_t>();
	int16_t sCapeID = pkt.read<int16_t>();

	int iLevel = pkt.read<uint8_t>(); // ·¹º§...
	...
	float fYObject = ACT_WORLD->GetHeightNearstPosWithShape(__Vector3(fXPos, fYPos, fZPos), 1.0f); // ¿ÀºêÁ§Æ®¿¡¼­ °¡Àå °¡±î¿î ³ôÀÌ°ª ¾ò±â..
	if (fYObject > fYTerrain) fYPos = fYObject;
	else fYPos = fYTerrain;

	int iFace = pkt.read<uint8_t>(); // ¸Ó¸®Ä«¶ô..
	int iHair = pkt.read<uint8_t>(); // ¾ó±¼ ¸ð¾ç

	int iStatus = pkt.read<uint8_t>(); // 1 - ¼­ÀÖ±â.. 2 - ¾É¾ÆÀÖ±â.. 3 ... Á×¾îÀÖ´Ù..

	// NOTE(srmeier): updating status
	//int iStatusSize = pkt.read<uint8_t>(); // 0 - º¸Åë Å©±â, 1 - Ä¿Á® ÀÖ´Ù. 2 - ÀÛ¾ÆÁ³´Ù..
	int iStatusSize = pkt.read<uint32_t>();

	int iRecruitParty = pkt.read<uint8_t>(); // 1 - º¸Åë. 2 - ÆÄÆ¼ ±¸ÇÔ..
	uint8_t byAuthority = pkt.read<uint8_t>(); // ±ÇÇÑ...
	...
	uint32_t	dwItemIDs[MAX_ITEM_SLOT_OPC];  // Âø¿ë ¾ÆÀÌÅÛ - ´Ù¸¥ ÇÃ·¹ÀÌ¾î(NPC Æ÷ÇÔ) 0 ~ 4 »óÃ¼,ÇÏÃ¼,Çï¸ä,ÆÈ,¹ß 5 ¸ÁÅä 6 ¿À¸¥¼Õ 7 ¿Þ¼Õ
	int		iItemDurabilities[MAX_ITEM_SLOT_OPC]; // Âø¿ë ¾ÆÀÌÅÛÀÇ ³»±¸·Â..
	for (int i = 0; i < MAX_ITEM_SLOT_OPC; i++)
	{
		dwItemIDs[i] = pkt.read<uint32_t>();			// Âø¿ëÇÏ°í ÀÖ´Â ¾ÆÀÌÅÛµéÀÇ ID
		iItemDurabilities[i] = pkt.read<int16_t>();	// Âø¿ëÇÏ°í ÀÖ´Â ¾ÆÀÌÅÛµéÀÇ ÇöÀç ³»±¸·Â
	...
	D3DCOLOR crID = !s_pPlayer->m_InfoBase.IsFriendlyNation(eNation) ? D3DCOLOR_XRGB(255, 96, 96) : D3DCOLOR_XRGB(128, 128, 255); // ±¹°¡¿¡ µû¶ó ´Ù¸¥»ö Àû¿ë
	...
	else if (0x02 == iStatusSize) pUPC->ScaleSetGradually(2.0f); // Ä¿Á³´Ù..
	else if (0x03 == iStatusSize) pUPC->ScaleSetGradually(0.5f);// ÀÛ¾ÆÁ³´Ù.
	else if (0x04 == iStatusSize) pUPC->FlickerFactorSet(0.7f); // ºÎÈ°µÇ¼­ ¹ÝÅõ¸í.. ±ô¹Ú°Å¸°´Ù..
...
bool CGameProcMain::MsgRecv_UserOut(Packet& pkt)
{
	...
	if (pUPC->IsDead())
	{//Ä³¸¯ÅÍ°¡ Á×Àº »óÅÂ¿¡¼­ UserOutÀ» ÇÏ°Ô µÇ¸é ½ÃÃ¼·Î ¹Ù²ãÁØ´Ù.
...
bool CGameProcMain::MsgRecv_UserInAndRequest(Packet& pkt)
{
	...
	for (int i = 0; i < iUPCCountReceived; i++)
	{
		...
		//		TRACE("               ID : %d\n", iID);
		if (iID == s_pPlayer->IDNumber()) continue; // ³» ¾ÆÀÌµðÀÌ´Ù.. °í·Î ³ÖÀ¸¸é ¾ÈµÈ´Ù.
	...
	if (iNewUPCCount > 0)
	{
		...
		std::vector<uint8_t> byBuff(iNewUPCCount * 2 + 10, 0);					// ÆÐÅ¶ ¹öÆÛ..
		CAPISocket::MP_AddByte(&(byBuff[0]), iOffset, WIZ_REQ_USERIN);	// Ä¿¸àµå.
...
bool CGameProcMain::MsgRecv_UserInRequested(Packet& pkt)
{
	...
	{
		this->MsgRecv_UserIn(pkt); // ÇÃ·¹ÀÌ¾î °¹¼ö ¸¸Å­ À¯Àú ÀÎ...
	}
...
bool CGameProcMain::MsgRecv_NPCInOut(Packet& pkt)
{
	...
	{
		return this->MsgRecv_NPCIn(pkt); // NPC In Ã³¸®
	}
	else // NPC ³ª°¥¶§.
	{
...
bool CGameProcMain::MsgRecv_NPCIn(Packet& pkt)
{
	...
	int		iID = pkt.read<int16_t>(); // Server¿¡¼­ °ü¸®ÇÏ´Â °íÀ¯ ID
	int		iIDResrc = pkt.read<int16_t>(); // ¸®¼Ò½º ID
	int		iType = pkt.read<uint8_t>();	// NPC Type - 0x05 : »óÀÎ
	int		iItemTrdeID = pkt.read<uint32_t>();	// ¾ÆÀÌÅÛ °Å·¡ÇÒ ±×·ì ID ¼­¹ö¿¡ ¿äÃ»ÇÒ ID
	int		iScale = pkt.read<int16_t>(); // ½ºÄÉÀÏ 100 Àº 1.0 
	int		iItemID0 = pkt.read<uint32_t>(); // ¸®¼Ò½º ID
	int		iItemID1 = pkt.read<uint32_t>(); // ¸®¼Ò½º ID
	int 	iNameLen = pkt.read<uint8_t>();
	...
	uint32_t dwStatus = pkt.read<uint32_t>(); // »óÅÂ... ¿©·¯°¡Áö·Î or ¿¬»êÇØ¼­ ¾´´Ù. 0 ¹® ¿­¸², 1 ´ÝÈû. 2, 4, 8, 16 ....
	uint32_t dwType = pkt.read<uint8_t>(); // Å¸ÀÔ... 0 ÀÌ¸é Ä³¸¯ÅÍ Å¸ÀÔ NPC, 1 ÀÌ¸é ¿ÀºêÁ§Æ® Å¸ÀÔ NPC
...
bool CGameProcMain::MsgRecv_NPCInAndRequest(Packet& pkt)
{
	...
	{
		int iOffset = 0;														// ¹öÆÛÀÇ ¿ÀÇÁ¼Â..
		std::vector<uint8_t> byBuff(iNewNPCCount * 2 + 10, 0);					// ÆÐÅ¶ ¹öÆÛ..
		CAPISocket::MP_AddByte(&(byBuff[0]), iOffset, WIZ_REQ_NPCIN);	// Ä¿¸àµå.
...
bool CGameProcMain::MsgRecv_NPCInRequested(Packet& pkt)
{
	...
	{
		this->MsgRecv_NPCIn(pkt); // ÇÃ·¹ÀÌ¾î °¹¼ö ¸¸Å­ À¯Àú ÀÎ...
	}
...
{
	int iType = pkt.read<uint8_t>(); // 0x01 - ¹°¸®ÀûÀÎ °ø°Ý, 0x02 ¸¶¹ý °ø°Ý 0x03, Áö¼Ó ¸¶¹ý °ø°Ý
	int iResult = pkt.read<uint8_t>(); // 0x00 ½ÇÆÐ, 0x01 ¼º°ø
	int iIDAttacker = pkt.read<int16_t>(); // °ø°ÝÇÑ ³Ñ
	int iIDTarget = pkt.read<int16_t>(); // Å¸°ÙÀÌ µÇ¼­ °ø°Ý´çÇÑ³Ñ.

	if (iIDAttacker == iIDTarget) return false;		// °ø°ÝÀÚ¿Í ÇÇÇØÀÚ°¡ °°Àº °æ¿ì????
	...
	if (bIAmAttacker) pAttacker = s_pPlayer;	// °ø°ÝÇÏ´Â ³ÑÀÌ ³» ÀÚ½ÅÀÌ¸é
	...
	else // Å¸°ÙÀÌ ´Ù¸¥³ÑÀÌ¸é..
	{
		...
		if (NULL == pTarget) pTarget = s_pOPMgr->CharacterGetByID(iIDTarget, false); // ¾ø´Ù¸é Á×¾î°¡´Â ³Ñµµ »ó°ü¾øÀÌ Å¸°ÙÀ¸·Î Àâ°í
	}
	...
	{
		if (pTarget->IsAlive() && 0 == pTarget->m_iSkillStep) // Á×Àº ³ÑÀÌ ¾Æ´Ï°í ½ºÅ³À» ¾²Áð ÁßÀÌ ¾Æ´Ï¸é ¸·´Â µ¿ÀÛÀ» ÇÑ´Ù..
		{
	...
	else if (0x2 == iResult) // Attack And Dead 
	{
		...
		if (pTarget->m_pShapeExtraRef) s_pOPMgr->CharacterDelete(pTarget->IDNumber()); // ¿ÀºêÁ§Æ® ÀÏ¶§´Â ¾ø¾Ö¹ö¸°´Ù..!!
	}
...
bool CGameProcMain::MsgRecv_ItemMove(Packet& pkt)
{
	...
	if (bResult)
	{
		...
		if (pInfoBase->iHP > pInfoBase->iHPMax) pInfoBase->iHP = pInfoBase->iHPMax; // ¹üÀ§°Ë»ç..
		if (pInfoExt->iMSP > pInfoExt->iMSPMax) pInfoExt->iMSP = pInfoExt->iMSPMax; // ¹üÀ§°Ë»ç..
...
bool CGameProcMain::MsgRecv_ItemWeightChange(Packet& pkt)		// ¾ÆÀÌÅÛ ¹«°Ô º¯È­..
{
	__InfoPlayerMySelf* pInfoExt = &(s_pPlayer->m_InfoExt);
...
bool CGameProcMain::MsgRecv_UserLookChange(Packet& pkt)
{
	...
	if (ePartPos != PART_POS_UNKNOWN)
	{
		...
		{
			__TABLE_PLAYER_LOOKS* pLooks = s_pTbl_UPC_Looks.Find(pUPC->m_InfoBase.eRace);	// User Player Character Skin ±¸Á¶Ã¼ Æ÷ÀÎÅÍ..
			if (NULL == pLooks)
			{
			...
			{
				if (PART_POS_HAIR_HELMET == ePartPos) pUPC->InitHair(); // ¸Ó¸®ÀÎµ¥ ¾ø´Â °æ¿ì´Â..
	...
	else if (ePlugPos != PLUG_POS_UNKNOWN)
	{
		...
		if (fDist < SOUND_RANGE_TO_SET) pUPC->SetSoundPlug(pItem); // SOUND_RANGE ¾È¿¡ ÀÖÀ¸¸é.
...
bool CGameProcMain::MsgRecv_ItemBundleDrop(Packet& pkt)		// Item ÀÌ ÇÊµå¿¡ ³ªÅ¸³ª´Âµ¥¿¡ ´ëÇÑ ÀÀ´ä
{
	int iID = pkt.read<int16_t>();
...
bool CGameProcMain::MsgRecv_ItemBundleOpen(Packet& pkt)		// ¾ÆÀÌÅÛ »óÀÚ¸¦ ¿­°Å³ª ½ÃÃ¼¸¦ µÚÁø´Ù..
{
	uint32_t dwItemID = 0;
	int iItemCount = 0;
...
bool CGameProcMain::MsgRecv_MyInfo_LevelChange(Packet& pkt)
{
	...
	if (iID == s_pPlayer->IDNumber())
	{
		...
		pInfoExt->iBonusPointRemain = pkt.read<uint8_t>(); // ³²Àº º¸³Ê½º Æ÷ÀÎÆ®..

		uint8_t	bExtraSkillPoint = pkt.read<uint8_t>();	// ÅäÅ» Æ÷ÀÎÆ®
...
void CGameProcMain::MsgRecv_MyInfo_RealmPoint(Packet& pkt)
{
	...
	if (bType == 1) // TODO: @Demircivi: LOYALTY_NATIONAL_POINTS is: 1, define it in header. 
	{
		...
		if (m_pUIVar->m_pPageState)
			m_pUIVar->m_pPageState->UpdateRealmPoint(s_pPlayer->m_InfoExt.iRealmPoint, s_pPlayer->m_InfoExt.iRealmPointMonthly); // ±¹°¡ ±â¿©µµ´Â 10À» ³ª´©¾î¼­ Ç¥½Ã
	}
	else if (bType == 2)
	{
...
void CGameProcMain::MsgRecv_MyInfo_PointChange(Packet& pkt)
{
	...
	if (iType >= 1 && iType <= 5)
	{
		...
		m_pUIVar->m_pPageState->UpdateBonusPointAndButtons(s_pPlayer->m_InfoExt.iBonusPointRemain); // º¸³Ê½º Æ÷ÀÎÆ® Àû¿ëÀÌ °¡´ÉÇÑ°¡??
	}
}
...
void CGameProcMain::InitUI()
{
...
#ifdef _DEBUG
	m_pUIStateBarAndMiniMap->SetPos(0, 70); // µð¹ö±× Á¤º¸ Ç¥½Ã¶§¹®¿¡ Á¶±Ý ³»¸°´Ù....
#else
	...
	m_pUICmdEditDlg->SetStyle(UISTYLE_ALWAYSTOP);
...
void CGameProcMain::MsgRecv_TargetHP(Packet& pkt)
{
	...
	uint8_t byUpdateImmediately = pkt.read<uint8_t>();	// 0x00 - Á¡Â÷ ´Ã¾î³ª°Ô²û.. 0x01 - Áï½Ã ¾÷µ¥ÀÌÆ®..
...
void CGameProcMain::MsgSend_NPCInRequest(int iID) // NPC Á¤º¸°¡ ¾øÀ» °æ¿ì ¿äÃ»ÇÑ´Ù..
{
	...
	int iOffset = 0;													// ¹öÆÛÀÇ ¿ÀÇÁ¼Â..
	uint8_t byBuff[32];;												// ÆÐÅ¶ ¹öÆÛ..
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_REQ_NPCIN);		// Ä¿¸àµå.
...
void CGameProcMain::MsgSend_UserInRequest(int iID) // User Á¤º¸°¡ ¾øÀ» °æ¿ì ¿äÃ»ÇÑ´Ù..
{
	...
	int iOffset = 0;													// ¹öÆÛÀÇ ¿ÀÇÁ¼Â..
	uint8_t byBuff[32];;												// ÆÐÅ¶ ¹öÆÛ..
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_REQ_USERIN);	// Ä¿¸àµå.
	CAPISocket::MP_AddShort(byBuff, iOffset, 1);					// ¾ÆÀÌµð °¹¼ö..
	CAPISocket::MP_AddShort(byBuff, iOffset, iID);					// ÀÚ¼¼ÇÑ Á¤º¸°¡ ÇÊ¿äÇÑ ¾ÆÀÌµðµé..
...
void CGameProcMain::MsgSend_Warp() // ¿öÇÁ - Á¸ÀÌµ¿ÀÌ µÉ¼öµµ ÀÖ´Ù..
{
	...
	CAPISocket::MP_AddShort(byBuff, iOffset, WI.iID); // ¿öÇÁ ¾ÆÀÌµð º¸³»±â...
	s_pSocket->Send(byBuff, iOffset);
...
bool CGameProcMain::MsgRecv_ItemTradeStart(Packet& pkt)			// ¾ÆÀÌÅÛ »ó°Å·¡..
{
...
bool CGameProcMain::MsgRecv_ItemTradeResult(Packet& pkt)			// ¾ÆÀÌÅÛ »ó°Å·¡ °á°ú..
{
...
{
	if (m_pSnd_Battle) m_pSnd_Battle->Stop(0.0f); // À½¾Ç ¸ØÃß±â..
	...
	if (iZonePrev != iZone) // ´Ù¸¥ Á¸ÀÏ °æ¿ì¿¡¸¸ ·Îµù..
	{
		...
		if (m_pUIChatDlg) m_pUIChatDlg->ChangeChattingMode(N3_CHAT_NORMAL); //Á¸ ÀÌµ¿À» ÇÏ¸é ÀÏ¹Ý Ã¤ÆÃ»óÅÂ·Î ¹Ù²ãÁØ´Ù.
		if (m_pUIPartyOrForce) m_pUIPartyOrForce->MemberDestroy();	//Á¸ ÀÌµ¿À» ÇÒ¶§ ÆÄÆ¼¸¦ ±ü´Ù...
		...
		float fZoom = 6.0f;
		e_Class_Represent eCR = CGameProcedure::GetRepresentClass(s_pPlayer->m_InfoBase.eClass);
		if (CLASS_REPRESENT_ROGUE == eCR) fZoom = 3.0f; // ·Î±× °è¿­Àº ¸ÊÀÌ Á»´õ ³Î¸® ÀÚ¼¼È÷ º¸ÀÎ´Ù..
...
void CGameProcMain::MsgSend_GameStart()
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_GAMESTART);						// °ÔÀÓ ½ºÅ¸Æ® ÆÐÅ¶ Ä¿¸àµå..
...
void CGameProcMain::MsgRecv_UserState(Packet& pkt)
{
	...
	else if (N3_SP_STATE_CHANGE_SIZE == eSP) // Å©±â º¯ÇÔ
	{
		...
		else if (0x02 == iState) pBPC->ScaleSetGradually(2.0f); // Ä¿Á³´Ù..
		else if (0x03 == iState) pBPC->ScaleSetGradually(0.5f);// ÀÛ¾ÆÁ³´Ù.
		else if (0x04 == iState) pBPC->FlickerFactorSet(0.7f); // ºÎÈ°µÇ¼­ ¹ÝÅõ¸í.. ±ô¹Ú°Å¸°´Ù..
	}
	else if (N3_SP_STATE_CHANGE_ACTION == eSP) // Å©±â º¯ÇÔ
	{
		if (1 == iState) pBPC->AnimationAdd(ANI_GREETING0, true); // ÀÎ»ç
		else if (2 == iState) pBPC->AnimationAdd(ANI_GREETING1, true); // ÀÎ»ç
		else if (3 == iState) pBPC->AnimationAdd(ANI_GREETING2, true); // ÀÎ»ç
		else if (11 == iState) pBPC->AnimationAdd(ANI_WAR_CRY1, true); // µµ¹ß
		else if (12 == iState) pBPC->AnimationAdd(ANI_WAR_CRY2, true); // µµ¹ß
		else if (13 == iState) pBPC->AnimationAdd(ANI_WAR_CRY3, true); // µµ¹ß
	}
...
void CGameProcMain::MsgRecv_PartyOrForce(Packet& pkt)
{
	...
	case N3_SP_PARTY_OR_FORCE_INSERT:			// 0x02	// Send - s1(ID) | Recv - s3(ID, HPMax, HP) b2(Level, Class) - ¹®ÀÚ¿­Àº ID ·Î ¾Ë¾Æ³½´Ù..
	{
		...
		else // ÆÄÆ¼¿¡ µé¾î¿Ã¼ö ¾ø´Ù..
		{
			...
			if (-1 == iErrorCode) ::_LoadStringFromResource(IDS_PARTY_INSERT_ERR_REJECTED, szMsg); // »ó´ë¹æÀÌ ÆÄÆ¼¿¡ µé¾î¿À±â¸¦ °ÅÀý ÇÏ¿´´Ù..
			else if (-2 == iErrorCode) ::_LoadStringFromResource(IDS_PARTY_INSERT_ERR_LEVEL_DIFFERENCE, szMsg); // ·¹º§ Â÷ÀÌ°¡ ³Ê¹« ³­´Ù...
			else if (-3 == iErrorCode) ::_LoadStringFromResource(IDS_PARTY_INSERT_ERR_INVALID_NATION, szMsg); // ÆÄÆ¼¸¦ ¸ÎÀ» ¼ö ¾ø´Â ±¹°¡ÀÌ´Ù.
			else ::_LoadStringFromResource(IDS_PARTY_INSERT_ERR, szMsg); // »ó´ë¹æÀÌ ÆÄÆ¼¿¡ µé¾î¿À±â¸¦ °ÅÀý ÇÏ¿´´Ù..

			this->MsgOutput(szMsg, D3DCOLOR_ARGB(255, 255, 255, 255));
			if (m_pUIPartyOrForce->MemberCount() == 1) m_pUIPartyOrForce->MemberDestroy(); // ¸â¹ö°¡ ÇÑ¸íÀÌ¸é ³»°¡ ÆÄÆ¼¸¦ ¸¸µç °æ¿ì´Ù.
		}
	...
	case N3_SP_PARTY_OR_FORCE_REMOVE:			// 0x03	// Send - s1(ID) | Recv - s1(ID) - 
	{
		...
		if (iID == s_pPlayer->IDNumber())
		{
			std::string szMsg; ::_LoadStringFromResource(IDS_PARTY_DESTROY, szMsg); // ÆÄÆ¼¸¦ ¶°³µ´Ù..
		...
		else
		{
			...
			if (pInfo)
			{
				std::string szMsg; ::_LoadStringFromResource(IDS_PARTY_LEAVE, szMsg); // ÆÄÆ¼¸¦ ¶°³µ´Ù..
				this->MsgOutput(pInfo->szID + szMsg, D3DCOLOR_ARGB(255, 255, 255, 255)); // ´©°¡ ÆÄÆ¼¿¡¼­ ¶°³µ´Ù´Â ¸Þ½ÃÁö..
...
void CGameProcMain::MsgSend_StateChange(e_SubPacket_State eSP, int iState)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_STATE_CHANGE);	// »óÅÂ º¯È­..
	CAPISocket::MP_AddByte(byBuff, iOffset, eSP);
...
void CGameProcMain::MsgSend_PerTradeReq(int iDestID, bool bNear)
{
	...
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)iDestID);			// »ó´ë¹æ ¾ÆÀÌµð..
	if (bNear)
...
void CGameProcMain::TargetSelect(CPlayerNPC* pTarget)
{
	...
	{ //¿î¿µÀÚ´Â Æ÷Ä¿½º¸¦ ÁÖÁö ¾Ê°Ô ÇÏ±â À§ÇØ¼­...
		...
		if (iIDTargetPrev != s_pPlayer->m_iIDTarget) // ÀüÀÇ Å¸°Ù°ú ´Ù¸£¸é
		{
			...
			{
				if (!s_pPlayer->m_InfoBase.IsFriendlyNation(pTarget->m_InfoBase.eNation)) crID = 0xffff4040; // ´Ù¸¥ ±¹°¡ÀÌ¸é
				else crID = 0xff6b9fff;
			}
			else // NPC
			{
				if (!s_pPlayer->m_InfoBase.IsFriendlyNation(pTarget->m_InfoBase.eNation)) crID = 0xffff6060; // ´Ù¸¥ ±¹°¡ÀÌ¸é
...
void CGameProcMain::MsgRecv_SkillChange(Packet& pkt)			// ½ºÅ³ º¯È­..
{
	int iType = pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_ClassChange(Packet& pkt)			// Á÷¾÷ º¯È­..
{
...
void CGameProcMain::MsgRecv_ObjectEvent(Packet& pkt)
{
	...
	{
		int iID = pkt.read<int16_t>();	// ¿­°í ´ÝÀ» ¼º¹® ID
		int iActivate = pkt.read<uint8_t>();	// ¿­°í ´ÝÀ½..
...
void CGameProcMain::ParseChattingCommand(const std::string& szCmd)
{
	...
	case CMD_WHISPER:
	{
		this->MsgSend_ChatSelectTarget(szCmds[1]); // ÀÏ´ëÀÏ Ã¤ÆÃ »ó´ë Á¤ÇÏ±â.
	}
	break;

	case CMD_TOWN:
	{
		if (s_pPlayer->m_bStun) return; // ±âÀýÇØ ÀÖÀ½ ¸øÇÔ..
		if (s_pPlayer->m_InfoBase.iHP * 2 >= s_pPlayer->m_InfoBase.iHPMax) // HP°¡ ¹Ý ÀÌ»ó ÀÖ¾î¾ß ÇÑ´Ù.
		{
			...
			int iOffset = 0;
			CAPISocket::MP_AddWord(byBuff, iOffset, WIZ_HOME);		// ¸¶À»·Î °¡±â...
			s_pSocket->Send(byBuff, iOffset);
		}
		else // HP°¡ ¹Ý ÀÌ»ó ÀÖ¾î¾ß ÇÑ´Ù.
		{
			std::string szMsg;
	...
	case CMD_TRADE:
	{
		...
		if (pOPC &&
			...
			!m_pUITransactionDlg->IsVisible()) // Å¸°ÙÀ¸·Î ´Ù¸¥ ÇÃ·¹ÀÌ¾î°¡ ÀâÇôÀÖ°í..  °¡±îÀÌ ÀÖÀ¸¸é.. // °³ÀÎ°£ ¾ÆÀÌÅÛ °Å·¡.. // »ó°Å·¡ ÁßÀÌ ¾Æ´Ï¸é..
		{
	...
	case CMD_PARTY:
	{
		...
		if (pTarget)
		{
			...
			if (this->MsgSend_PartyOrForceCreate(0, pTarget->IDString()))
				::_LoadStringFromResource(IDS_PARTY_INVITE, szMsg); // ÆÄÆ¼ ¿äÃ».. 
			else
				::_LoadStringFromResource(IDS_PARTY_INVITE_FAILED, szMsg); // ÆÄÆ¼ ÃÊ´ë ½ÇÆÐ
	...
	case CMD_LEAVEPARTY:
	{
		this->MsgSend_PartyOrForceLeave(0); // ÆÄÆ¼ ¿äÃ»..
	}
	break;
	...
	case CMD_NOTICE:
	{
		...
		{
			std::string szChat = szCmd.substr(s_szCmdMsg[CMD_NOTICE].size() + 2); // "/°øÁö "¸¦ Á¦¿ÜÇÑ ³ª¸ÓÁö ¹®ÀÚ¿­
	...
	case CMD_ARREST:
	{
		this->MsgSend_Administrator(N3_SP_ADMINISTRATOR_ARREST, szCmds[1]); //ÃßÀû		
	}
	break;

	case CMD_FORBIDCONNECT:
	{
		this->MsgSend_Administrator(N3_SP_ADMINISTRATOR_FORBID_CONNECT, szCmds[1]); //Á¢¼Ó±ÝÁö		
	}
	break;

	case CMD_FORBIDCHAT:
	{
		this->MsgSend_Administrator(N3_SP_ADMINISTRATOR_CHAT_FORBID, szCmds[1]); //Ã¤ÆÃ±ÝÁö		
	}
	break;

	case CMD_PERMITCHAT:
	{
		this->MsgSend_Administrator(N3_SP_ADMINISTRATOR_CHAT_PERMIT, szCmds[1]); //Ã¤ÆÃÇã°¡		
	}
	break;
...
void CGameProcMain::UpdateUI_MiniMap()
{
	...
	for (; it != itEnd; it++) // NPC
	{
		...
		if (!pNPC->m_InfoBase.IsFriendlyNation(eNation)) crType = 0xff800000; // ´Ù¸¥ ±¹°¡ NPC È¤Àº ¸ó½ºÅÍ ÁÖÈ²»ö
	...
	for (; it2 != itEnd2; it2++) // User
	{
		...
		bool bDrawTop = false;
		if (!pUPC->m_InfoBase.IsFriendlyNation(eNation)) // Àû±¹ÀÏ°æ¿ì
		{
...
void CGameProcMain::MsgRecv_DurabilityChange(Packet& pkt)		// ³»±¸·Â º¯°æ..
{
	e_ItemSlot eSlot = (e_ItemSlot)pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_KnightsListBasic(Packet& pkt) // ±â»ç´Ü ±âº» Á¤º¸ ¹Þ±â..
{
	...
	{
		int iCount = pkt.read<int16_t>();	// ±â»ç´Ü °¹¼ö
		for (int i = 0; i < iCount; i++)
		{
			std::string szID;
			int iID = pkt.read<int16_t>();		// ±â»ç´Ü ID
			int iLen = pkt.read<int16_t>();		// ID ¹®ÀÚ¿­ ±æÀÌ..
			pkt.readString(szID, iLen);	// ID ¹®ÀÚ¿­..
...
void CGameProcMain::MsgRecv_ContinousPacket(Packet& pkt) // ¾ÐÃàµÈ µ¥ÀÌÅÍ ÀÌ´Ù... ÇÑ¹ø ´õ ÆÄ½ÌÇØ¾ß ÇÑ´Ù!!!
{
	uint16_t iWholeSize;
	pkt >> iWholeSize;
...
void CGameProcMain::MsgRecv_WareHouse(Packet& pkt)			// º¸°üÇÔ °ü·Ã ÆÐÅ¶..
{
...
void CGameProcMain::MsgRecv_WareHouseOpen(Packet& pkt)		// º¸°üÇÔ ¿ÀÇÂ..
{
	if (m_pUIWareHouseDlg->IsVisible())
		return;
...
void CGameProcMain::MsgRecv_NpcChangeOpen(Packet& pkt)		// Class Change¿Í ÃÊ±âÈ­..
{
	uint8_t	bSubCom = pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_AllPointInit(Packet& pkt)			// All Point ÃÊ±âÈ­..
{
	uint8_t	bType = pkt.read<uint8_t>();
	...
	case 0x01:	// ¼º°ø..
		...
		s_pPlayer->m_InfoExt.iBonusPointRemain = pkt.read<int16_t>(); // ³²Àº º¸³Ê½º Æ÷ÀÎÆ®..
		m_pUIVar->m_pPageState->UpdateBonusPointAndButtons(s_pPlayer->m_InfoExt.iBonusPointRemain); // º¸³Ê½º Æ÷ÀÎÆ® Àû¿ëÀÌ °¡´ÉÇÑ°¡??
...
void CGameProcMain::MsgRecv_SkillPointInit(Packet& pkt)		// Skill Point ÃÊ±âÈ­..
{
	uint8_t	bType = pkt.read<uint8_t>();
	uint32_t	dwGold = pkt.read<uint32_t>();
	int i;
...
void CGameProcMain::MsgRecv_PointChangePriceQueryRequest(Packet& pkt)		// °¡°Ý¿¡ ´ëÇÑ ÀÀ´ä ÆÐÅ¶..
{
	uint32_t	dwGold = pkt.read<uint32_t>();
	m_pUINpcChange->ReceivePriceFromServer(dwGold);
}

void CGameProcMain::MsgRecv_NoahChange(Packet& pkt)		// ³ë¾Æ º¯°æ..
{
	uint8_t	bType = pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_WarpList(Packet& pkt)		// ¿öÇÁ ¸®½ºÆ® - Á¸ Ã¼ÀÎÁö°¡ µÉ ¼öµµ ÀÖ´Ù..
{
	...
	for (int i = 0; i < iListCount; i++)
	{
		...
		WI.iID = pkt.read<int16_t>(); // ¿öÇÁ ID
		iStrLen = pkt.read<int16_t>(); // ÀÌ¸§ ±æÀÌ
		pkt.readString(WI.szName, iStrLen); // ÀÌ¸§
		iStrLen = pkt.read<int16_t>(); // µ¿ÀÇ¹® ±æÀÌ
		pkt.readString(WI.szAgreement, iStrLen); // µ¿ÀÇ¹®
		WI.iZone = pkt.read<int16_t>();				// Á¸¹øÈ£
		WI.iMaxUser = pkt.read<int16_t>();			// ÃÖ´ë À¯Àú Ä«¿îÆ®.
		WI.iGold = pkt.read<uint32_t>();				// µ·
		WI.vPos.x = (pkt.read<int16_t>()) / 10.0f;	// ÁÂÇ¥ 
...
/*
void CGameProcMain::MsgRecv_ServerCheckAndRequestConcurrentUserCount(Packet& pkt)	// ¼­¹ö IP ¿Í Æ÷Æ®¸¦ ¹Þ¾Æ µ¿Á¢ÀÚ¸¦ Ã¼Å©ÇØ º»´Ù..
{
	std::string szIP;
...
void CGameProcMain::MsgRecv_ConcurrentUserCountAndSendServerCheck(Packet& pkt)			// µ¿Á¢ÀÚ¸¦ ¹Þ°í ¼­¹ö¿¡ Á¢¼ÓÇÏ°Ú´Ù´Â ÆÐÅ¶À» º¸³½´Ù.
{
...
void CGameProcMain::MsgRecv_Knights_Create(Packet& pkt)
{
	...
	case N3_SP_KNIGHTS_CREATE_SUCCESS:
	{
		...
		int iLen = pkt.read<int16_t>();		// ID ¹®ÀÚ¿­ ±æÀÌ..
		pkt.readString(szID, iLen);	// ID ¹®ÀÚ¿­..
		int iGrade = pkt.read<uint8_t>();	// µî±Þ
		int iRank = pkt.read<uint8_t>();		// ¼øÀ§
		uint32_t dwGold = pkt.read<uint32_t>();
...
void CGameProcMain::MsgRecv_Knights_Join(Packet& pkt)
{
	...
	case N3_SP_KNIGHTS_COMMON_SUCCESS: //Å¬·£°¡ÀÔ ¼º°ø
	{
		...
		int iID = pkt.read<int16_t>();
		e_KnightsDuty eDuty = (e_KnightsDuty)pkt.read<uint8_t>();
		int iL = pkt.read<int16_t>(); // ¼Ò¼Ó ±â»ç´Ü ÀÌ¸§ ±æÀÌ.
		std::string szKnightsName;
...
bool CGameProcMain::MsgRecv_CharacterSelect(Packet& pkt) // virtual
{
	...
	if (s_pPlayer->IsDead())
	{
		this->InitPlayerPosition(s_pPlayer->Position()); // ÇÃ·¹ÀÌ¾î À§Ä¡ ÃÊ±âÈ­.. ÀÏÀ¸ÄÑ ¼¼¿ì°í, ±âº»µ¿ÀÛÀ» ÃëÇÏ°Ô ÇÑ´Ù.
		s_pPlayer->RegenerateCollisionMesh(); // Ãæµ¹ ¸Þ½Ã¸¦ ´Ù½Ã ¸¸µç´Ù..
		s_pPlayer->m_iSendRegeneration = 0; // ÇÑ¹ø º¸³»¸é ´Ù½Ã Á×À»¶§±îÁö ¾Èº¸³»´Â ÇÃ·¡±×
		s_pPlayer->m_fTimeAfterDeath = 0; // ÇÑ¹ø º¸³»¸é ´Ù½Ã Á×À»¶§±îÁö ¾Èº¸³»´Â ÇÃ·¡±×
...
{
	int iID = pkt.read<int16_t>();//regenÇÏ°íÀÚ ÇÏ´Â À¯ÀúÀÇ ¾ÆÀÌµð
			...
			if (pShape && pShape == s_pPlayer->m_pObjectTarget && pShape->m_iEventID) // Event °¡ ÀÖÀ¸¸é..
			{
				...
				else
				{
					if (OBJECT_TYPE_BINDPOINT == pShape->m_iEventType)
					{
						...
						this->MessageBoxPost(szMsg, "", MB_YESNO, BEHAVIOR_REQUEST_BINDPOINT); // ¹ÙÀÎÆÃ Æ÷ÀÎÆ® ¼³Á¤ ¸Þ½ÃÁö ¹Ú½º
					}
...
{
	uint8_t	byBuff[10];											// ¹öÆÛ.. 
	int		iOffset = 0;											// ¿É¼Â..
	float	fTime = CN3Base::TimeGet();							// Å¬¶óÀÌ¾ðÆ® ½Ã°£

	s_pSocket->MP_AddByte(byBuff, iOffset, WIZ_SPEEDHACK_CHECK);	// ½ºÇÇµåÇÙ Ã¼Å© ÆÐÅ¶..
