...
void CN3FXBundleGame::Trigger(CN3Chr* source, __Vector3 TargetPos, int iSndID)
{
	m_bRegion = true;


	const __Matrix44* pMtx = source->MatrixGet(m_iSourceJoint);
	if (!pMtx) 
		m_vPos = source->Pos();
	else
	{
	...
	m_vDir = m_vDestPos - m_vPos;
	m_vDir.Normalize();
	CN3FXBundle::Trigger(1, -1, -1, iSndID);
}
...
void CN3FXBundleGame::SetPreBundlePos(CN3Chr* source, int iJoint)
{
	const __Matrix44* pMtx = source->MatrixGet(iJoint);
	if (!pMtx)
		m_vPrePos = source->Pos();
	else
	{
		const __Matrix44* pMtx = source->MatrixGet(iJoint);
		if (pMtx) {
			m_vPrePos = pMtx->Pos();
			m_vPrePos *= source->m_Matrix;
		}		
	}
